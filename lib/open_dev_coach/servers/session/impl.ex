    ...
    case Enum.find(configs, &(&1.key == key)) do
      nil ->
        ...
        if changeset.valid? do
          # Note: Config uses integer IDs (auto-generated by Ecto), so we don't provide an ID
          attrs = %{key: key, value: value, id: nil}
      ...
      existing_config ->
        ...
        # If config doesn't have an ID yet (just added but not persisted), replace it instead of updating
        if existing_config.id == nil do
          # Remove the old config and add a new one with updated value
          updated_configs = Enum.reject(configs, &(&1.key == key))
          temp_config = %Config{}
          changeset = Config.changeset(temp_config, %{key: key, value: value})

          if changeset.valid? do
            attrs = %{key: key, value: value, id: nil}
            new_config = struct(Config, attrs)
            updated_configs = [new_config | updated_configs]

            # Persist asynchronously
            if async_persistence?(),
              do: Task.start(fn -> Configuration.create_config(%{key: key, value: value}) end),
              else: Configuration.create_config(%{key: key, value: value})

            message = "Configuration '#{key}' set to '#{value}'"
            {{:ok, message}, %{state | configs: updated_configs}}
          else
            error_message = ChangesetHelper.format_changeset_errors(changeset)
            {{:error, error_message}, state}
          end
        else
          # Config has an ID, use normal update flow
          case update_in_memory_and_persist_async(
                 state,
                 existing_config,
                 attrs,
